...
export const FloatingDockMobile = ({
  ...
  return (
    <div
      className={cn("fixed bottom-4 left-4 z-50 block md:hidden", className)}
    >
      <AnimatePresence>
        {open && (
          <motion.div
...
export const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "fixed bottom-4 left-1/2 -translate-x-1/2 z-50 hidden h-16 items-end justify-center gap-4 rounded-2xl bg-gray-50 px-4 pb-3 md:flex dark:bg-neutral-900",
        className
      )}
    >
      {items.map((item) => (
